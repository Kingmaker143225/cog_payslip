...
export function numberToWords(num) {
  ...
  return `Rupees ${inWords(Math.round(n))} Only`;
}

export function buildPayslipData(row, index = 1, payMonth = "March", payYear = "2026") {
  const basic = Number(row["Basic"] || 0);
  const hra = Number(row["HRA"] || 0);
  const conveyance = Number(row["Conveyance"] || 0);
  const medical = Number(row["Medical"] || 0);
  const otherAllowance = Number(row["Other Allowance"] || 0);
  const specialAllowance = Number(row["Special Allowance"] || 0);

  const pf = Number(row["PF"] || 0);
  const esi = Number(row["ESI"] || 0);
  const profTax = Number(row["PROF TAX"] || 0);
  const tds = Number(row["TDS"] || 0);
  const otherDeductions = Number(row["Other Deductions"] || 0);

  const totalEarnings =
    Number(row["GROSS"] || basic + hra + conveyance + medical + otherAllowance + specialAllowance);

  const totalDeductions =
    Number(row["TOTAL DEDUCTIONS"] || pf + esi + profTax + tds + otherDeductions);

  const netPay =
    Number(row["NET PAY"] || totalEarnings - totalDeductions);

  return {
    companyName: "CognisysAI",
    companyAddress: "2nd Floor, Tech Park, Madhapur, Hyderabad - 500081",

    employeeId: row["Employee No"] || `EMP${index}`,
    employeeName: row["Name"] || `Employee ${index}`,
    doj: row["Joining Date"] || "N/A",
    designation: row["Designation"] || "N/A",
    department: row["Department"] || "N/A",
    location: row["Location"] || "N/A",
    paidDays: Number(row["Effective Work Days"] || 0),
    lop: Number(row["LOP"] || 0),

    bankName: row["Bank Name"] || "N/A",
    bankAccount: row["Bank Account No"] || "N/A",
    ifsc: row["IFSC Code"] || "N/A",
    pan: row["PAN Number"] || "N/A",
    pfNo: row["PF No"] || "N/A",
    pfUan: row["PF UAN"] || "N/A",
    ...
    totalDeductions,
    netPay,
    netPayWords: row["Net Pay In Words"] || numberToWords(netPay),
  };
}