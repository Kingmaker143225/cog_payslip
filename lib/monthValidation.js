...
export function validateSelectedMonthAgainstExcel(rows, selectedMonth, selectedYear) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const excelMonth = row["MONTH"];
    const excelYear = Number(row["YEAR"]);
    ...
    if (!excelYear) {
      return {
        valid: false,
        message: `YEAR value is missing in row ${i + 2}.`,
      };
    }

    const excelMonthNum = monthNameToNumber(excelMonth);

    if (!excelMonthNum) {
      return {
        valid: false,
        message: `Invalid MONTH value "${excelMonth}" in row ${i + 2}.`,
      };
    }

    if (excelMonthNum !== selectedMonthNum || excelYear !== selectedYearNum) {
      return {
        valid: false,
        message: `Month mismatch in row ${i + 2}. Excel has "${excelMonth} ${excelYear}" but you selected "${selectedMonth} ${selectedYear}".`,
      };
    }
  }
